...
function optOutToggle(userid,classid) {
	...
	params={
			...
			mode:mode,
			rnd:Math.random(),
			useCase: "updateOptOut"};
	finalURL=getAsUriParameters(params);

	jQuery.ajax({
		url: arg.url+"?"+finalURL,
		dataType: "jsonp",
		jsonp: 'callback',
...
function getAssistText(classid) {
	...
	jQuery.ajax({
		url: arg.url+"?"+finalURL,
//		url:"http://agile936.xavor.com:8090/AssistPlus//GetAssistText?classid=10141&workflowID=&workflowStatusID=&lifecycleID=Preliminary&userid=&objID=P00368&title=AJAXAgile+Product+Lifecycle+Management+-+Welcome%C2%A0%2C+Administrator%2C+admin+(admin)&affectedItem=&loadAssistForAffectedItem=false&loadAssistForMainObject=true&statistics=%7B%22userid%22%3A%22admin%22%2C%22main%22%3A%7B%22classid%22%3A%2211610%22%2C%22attrs%22%3A%7B%7D%7D%7D&rnd=0.938471290463611",
		dataType: "jsonp",
		jsonp: 'callback',
		jsonpCallback: 'jsonpCallback',
		success:function(response){
