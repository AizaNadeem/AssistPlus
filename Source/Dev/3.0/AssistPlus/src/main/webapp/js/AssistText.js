...
PLMFlex.Assist.AssistText = (function() {
	...
	function AddNotifText($containerHeader, $contentboxcontent) {
		...
		$contentboxcontent.html($containerHTML);
		$contentboxcontent.css('padding','0');
		$contentboxcontent.attr("id","loadClassNotification");
		$contentboxcontent.outerHeight($(window).height()-100);
		$("#loadClassNotification").css({
			'overflow': 'auto'
		});
		$('#sub-class').hide();
	...
	function drawRolesText(roles, roleOptions)
	{
	...
	{
		PLMFlex.Assist.BindEvents.TinyNotify("Please wait while we are loading Assist Class Notification","attention");
		$('#assistClassMessage').hide();
		...
		$('#sub-class').show();
		$('#sub-class').text(selectedClass.trim());
		$this.addClass('selected');
		PLMFlex.Assist.Request.get("ClassNotificationMsgBar?classId=" + classId + "&level="+ classLevel + "&isAdmin=true" ,function(jsonResponse){
			...
			var enableOverrideNotification = jsonResponse.isNotifOverrideEnabled;
			var fontC = jsonResponse.classFontColor;
			var backgroundC = jsonResponse.classBackgroundColor;
			if(fontC == 'null') {
				fontC = '#3e3434';
			}
			if(backgroundC == 'null') {
			...
			$("#notificationClassBackgroundColor").find('.colorSelector').remove();
			AssistColorPicker.createPicker($("#notificationClassBackgroundColor"),fontC,backgroundC,true,false);
			$("#notificationClassBackgroundColor").find('.picker').html("Default Color");	
			$("#notificationClassBackgroundColor").find('.picker').each(function() {
			...
			$("#dialog-notif-class-roles").html('');

			var $dialogRoles=$("#dialog-notif-class-roles");
			...
			if(enableNotification!=null && enableNotification!="null"){
				...
				$('#enableNotificationOverride input[type=radio][value=' + enableOverrideNotification + ']').prop('checked', true);
				drawRolesText(jsonResponse.classRoles.split(","),"#roleoptions");
				...
				$('#enableNotificationOverride input[type=radio][value="No"]').prop('checked', true);
				AssistColorPicker.createPicker($("#notificationClassBackgroundColor"),'#3e3434','#EEECE1',true,false);
				drawRolesText(jsonResponse.classRoles.split(","),"#roleoptions");
				$dialogRoles.find(".roleList").autolist();
				setTimeout(function() {
	      			setEditorContent(notificationMsg);
	...
	function bindClassActions($this)
	{
		...
		if((typeof classID!="undefined") && classID!="") {
			...
			var overrideNotifEnabled = $("input:radio[name='enableNotificationOverride']:checked").val();
			var fontColor = AssistColorPicker.getHexColor($('#notificationClassBackgroundColor .colorSelector button').css('color'));
			var backgroundColor = AssistColorPicker.getHexColor($('#notificationClassBackgroundColor .colorSelector button').css('background-color'));
			...
			if(notifEnabled!=null && notifEnabled!="null"){
				...
				PLMFlex.Assist.Request.post("ClassNotificationMsgBar",{
						...
						overrideEnable:overrideNotifEnabled,
						fontColor: fontColor,
					...
					function(jsonResponse) {
						if(jsonResponse && jsonResponse.status == 'success') {
							...
							if(notifEnabled == 'Yes') {
								...
								var classProperty = $('#class-table tr.selected').attr("class");
								var classValue = classProperty.match(/child-of-(\d+)/)[1];
								$('#'+classValue).find("#hasTextFlag").not(':has(#textImage)').append("<img id='textImage' src='img/dot.gif' alt='Has Assist Text' title='Has Assist Text' />");
								var classPropertyLast = $('#'+classValue).attr("class");
								var classValueLast = classPropertyLast.match(/child-of-(\d+)/)[1];
								$('#'+classValueLast).find("#hasTextFlag").not(':has(#textImage)').append("<img id='textImage' src='img/dot.gif' alt='Has Assist Text' title='Has Assist Text' />");
							} else {
								$('#class-table tr.selected #hasTextFlag').empty();
								var classProperty = $('#class-table tr.selected').attr("class");
								var classValue = classProperty.match(/child-of-(\d+)/)[1];
								var classSelected = classProperty.split(' ').slice(0, 2).join(' ');
								var textExists = false;
								$('#class-table').find('tr').each(function() {
									if($(this).attr("class").includes(classSelected) && !textExists) {
										if($(this).find("#hasTextFlag").find('#textImage').length > 0) {
											textExists = true;
										}
									}
								});
								if(!textExists) {
									$('#'+classValue).find("#hasTextFlag").empty();
								}
								var classPropertyLast = $('#'+classValue).attr("class");
								var classValueLast = classPropertyLast.match(/child-of-(\d+)/)[1];
								var classSelectedLast = classPropertyLast.split(" ").slice(0, 3).join(" ");
								var textExistsLast = false;
								$('#class-table').find('tr').each(function() {
									if($(this).attr("class").includes(classSelectedLast) && !textExistsLast) {
										if($(this).find("#hasTextFlag").find('#textImage').length > 0) {
											textExistsLast = true;
										}
									}
								...
								if(!textExistsLast) {
									$('#'+classValueLast).find("#hasTextFlag").empty();
								}
