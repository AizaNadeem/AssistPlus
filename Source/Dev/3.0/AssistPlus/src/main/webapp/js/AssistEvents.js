...
PLMFlex.Assist.BindEvents=(function(){
	...
	function saveAssistMsgSettings($button){
		$button.click(function(){
			PLMFlex.Assist.AssistMessageBar.Save($('.subcontent'));
		});
	}
	...
	function LoadAssistMessage($tr){
		$tr.click(function(){
		    ...
		    if(classLevel === 3)
				PLMFlex.Assist.AssistText.loadAssistMessage(classID,classLevel,$this);
		});
	}
...
function LongNotification(text,type,enable){
	...
	return{
		...
		saveAssistMsgSettings: saveAssistMsgSettings,
		saveLic:saveLicense,
...
$(document).ready(function() {
	...
	$('#addNotifText').click(function() {
		if(LicInfo)
			PLMFlex.Assist.AssistText.addNotif($contentboxheader, $contentboxcontent);
		else
			PLMFlex.Assist.BindEvents.Notify(licMsgs['invalid'],'info');
	});
	...
	$('#notificationMsg').click(function() {
		if(LicInfo)
			PLMFlex.Assist.AssistMessageBar.Load($contentboxheader, $contentboxcontent);
		else
			PLMFlex.Assist.BindEvents.Notify(licMsgs['invalid'],'info');
	});
	...
	$('#notif-usage-report').click(function() {
		PLMFlex.Assist.NotifUsageReport.loadReport($contentboxheader, $contentboxcontent);			
	});
	
	$('#user-ack-report').click(function() {
		PLMFlex.Assist.UserAckReport.loadReport($contentboxheader, $contentboxcontent);			
	});
