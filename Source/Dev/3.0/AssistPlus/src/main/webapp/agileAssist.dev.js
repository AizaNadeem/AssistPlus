 ...
 * version 2.3.1
 * Copyright 2021, Xavor.com
 */

//PLMXL is the global data container object
var PLMXL = {};
var NotificationMsg = true;
var classIdInUse = null;
var classOverride = true;
...
(function() {
	...
	PLMXL.toJSON = function(value) {
		return str("", {"": value});
	}
})();
function setNotificationMsg(response, main) {
	jQuery("#assistMsg").remove();
	jQuery("#assistAgileClassMsg").remove();
	var notificationMsg = "";
	var isAckEnable = "";
	var msgDiv = "";
	if(main) {
		var duration = response.durationLimit;
		if (response.isDurationEnabled=='Yes') {
			var diff = 0;
		...
		if((response.isDurationEnabled=='Yes' && diff > duration) || response.isNotifEnabled=="No"||response.notificationMsg=="" ||
				response.notificationMsg==undefined || response.notificationMsg==null||
				response.notificationMsg=='null' || response.isAckUser == "true"){
			NotificationMsg = false;
			jQuery("#assistDummyMsg").remove();
			return;
		}
		notificationMsg = response.notificationMsg;
		isAckEnable = response.isAckEnabled;
		msgDiv = "<div style='padding: 8px;' id='assistMsg'>";
		if(!NotificationMsg) {
			jQuery("#assistDummyMsg").remove();
			jQuery("#header_tab_wrapper").prepend("<div style='padding: 8px;' id='assistDummyMsg'>H</div>");
			NotificationMsg = true;
		}
	} else {
		if(response.isClassNotifEnabled=="No"||response.classNotificationMsg=="" ||
				response.classNotificationMsg==undefined || response.classNotificationMsg==null||
				response.classNotificationMsg=='null'){
			jQuery("#assistClassDummyMsg").remove();
			classMap.set(classIdInUse, [false, response.isNotifOverrideEnabled=="Yes"? true: false]);
			return;
		}
		notificationMsg = response.classNotificationMsg;
		isAckEnable = "No";
		msgDiv = "<div style='padding: 8px;' id='assistAgileClassMsg'>";
		
		if(classMap.has(classid) ? !classMap.get(classid)[0] : true) {
			jQuery("#assistClassDummyMsg").remove();
			jQuery("#header_tab_wrapper").prepend("<div style='padding: 8px;' id='assistClassDummyMsg'>H</div>");
			classMap.set(classIdInUse, [true, response.isNotifOverrideEnabled=="Yes"? true: false]);
		}
	}
	
	var firstSubstring = notificationMsg.match(/<p>.*?<\/p>/)[0];
	var remainingSubstring = notificationMsg.substring(firstSubstring.length);
	var match = notificationMsg.match(/<p>(.*?)<br \/>(.*?)<\/p>/);
	if(match && remainingSubstring == '') { 
		firstSubstring = match[1];
		remainingSubstring = match[2];
	} 
	var textContent = notificationMsg.replace(/<[^>]*>|&nbsp;|&amp;/g, '');
	var cleanTextContent = notificationMsg.replace(/<[^>]*>/g, '');
	const charCount = response.noOfChars - cleanTextContent.substring(0, response.noOfChars).replace(/&nbsp;|&amp;/g, '').length;
	const matchIndex = firstSubstring.indexOf(cleanTextContent);
	const maxChars = charCount + matchIndex + parseInt(response.noOfChars);
	
	if(remainingSubstring !== '' || textContent.length > response.noOfChars) {
		msgDiv = msgDiv + seeMoreDiv(firstSubstring.substring(0, maxChars)+" ...");
		msgDiv = msgDiv + popupDiv(notificationMsg, isAckEnable, main? 'popup-container': 'popup-container-agile-class') + "</div>";		
		setTimeout(function() {
			jQuery(main? '#assistDummyMsg': '#assistClassDummyMsg').remove();
			jQuery("#assistMsg").remove();
			jQuery("#header_tab_wrapper").prepend(msgDiv);	
			jQuery(main? '#assistMsg': '#assistAgileClassMsg').css("background-color",main? response.backgroundColor: response.classBackgroundColor)
														      .css("color",main? response.fontColor: response.classFontColor);
			jQuery('#see-more-div').css("color",main? response.fontColor: response.classFontColor);
			jQuery('#see-more-link').css({
					"text-decoration": "underline",
					"color": main? response.fontColor: response.classFontColor
			});
			...
			onClick('#see-more-link', main? '#assistMsg': '#assistAgileClassMsg', main? '#popup-container': '#popup-container-agile-class');
			if(isAckEnable=="Yes") {
				onAckClick('#accept-button', response.notificationId);
			}
			onClick('#close-button', main? '#assistMsg': '#assistAgileClassMsg', main? '#popup-container': '#popup-container-agile-class');
			ResizeGrid();
		}, 500);
	} else {
		msgDiv = msgDiv + buttonsDiv(isAckEnable) + notificationMsg + "</div>";
		setTimeout(function() {
			jQuery(main? '#assistDummyMsg': '#assistClassDummyMsg').remove();
			jQuery("#assistMsg").remove();
			jQuery("#header_tab_wrapper").prepend(msgDiv);
			jQuery(main? '#assistMsg': '#assistAgileClassMsg').css("background-color",main? response.backgroundColor: response.classBackgroundColor)
														      .css("color",main? response.fontColor: response.classFontColor);
			if(isAckEnable=="Yes") {
				onAckClick('#acknowledgeAssistMessage', response.notificationId);
			}
			onClick('#cancelAssistMessage', main? '#assistMsg': '#assistAgileClassMsg', main? '#popup-container': '#popup-container-agile-class');
			ResizeGrid(); 
		}, 500);	
	}
}

function onAckClick(button, notificationId) {
	var url = "/AssistPlus/AcknowledgeAssistMessage";
	var userid = document.title.substring(document.title.lastIndexOf("(")+1,document.title.lastIndexOf(")"));
	var userName = "";
	if(userid == "")
		userName = document.title.substring(document.title.lastIndexOf(',', document.title.lastIndexOf(',') - 1)+2);
	else
		userName = document.title.substring(document.title.lastIndexOf(',', document.title.lastIndexOf(',') - 1)+2, document.title.lastIndexOf("(")-1);
	jQuery(button).unbind('click');
	if(button === '#accept-button') {
...
function onClick(button, mainDiv, popupDiv) {
	jQuery(button).unbind('click');
	if(button === '#see-more-link') {
		jQuery(button).click(function(event) {
			event.preventDefault();
			jQuery(popupDiv).css('display', 'flex');
		});
	} else if(button === '#close-button') {
		jQuery(button).click(function() {
			jQuery(popupDiv).css('display', 'none');
			jQuery(mainDiv).hide();
			ResizeGrid();
		});
	} else if(button === '#cancelAssistMessage') {
		jQuery(button).click(function() {
			jQuery(mainDiv).hide();
			ResizeGrid();
		});	
...
function seeMoreDiv(notifMsg) {
	let div = "<div id='see-more-div' style='display: flex; align-items: center;'>"+
		notifMsg+
		"<a id='see-more-link' class='see-more' href='#'>See More</a>"+
	"</div>";
	return div;
}
	
function popupDiv(notifMsg, enableAck, popup) {
	let div = "<div id=" +popup+ " class='popup-container'>"+
			"<div id='popup-content' class='popup-content'>"+
			notifMsg+
			"<div class='button-container'>";
			if(enableAck=="Yes")
				div = div + "<button id='accept-button' class='popup-button'>Opt-Out</button>";
			div = div + "<button id='close-button' class='popup-button'>Close</button>"+
			"</div>"+
			"</div>"+
			"</div>";
	return div;
...
function buttonsDiv(enableAck){
	let buttons = "<button id='cancelAssistMessage' class='cancel-assist'>×</button>";
	if(enableAck=="Yes")
		buttons = buttons + "<button id='acknowledgeAssistMessage' class='acknowledge-assist'>Opt-Out</button>";
	return buttons;
}

function getNotificationMsg() {
	var arg = {};
	arg.url = "/AssistPlus/NotificationMsgBar?buster="+new Date().getTime()+"&isAdmin=false";
	var title = document.title;
	var userid = title.substring(title.lastIndexOf("(")+1,title.lastIndexOf(")"));
	jQuery.get(arg.url, {
			userid:userid},
		function(response) {
			if(response.isNotifEnabled=="No"||response.notificationMsg=="" ||
			response.notificationMsg==undefined || response.notificationMsg==null||
			response.notificationMsg=='null' || response.isAckUser == "true"){
				jQuery("#assistDummyMsg").remove();
				NotificationMsg = false;
		...
		NotificationMsg = true;
		setNotificationMsg(response, true);
...
function initPLMXL() {
	...
	PLMXL.HELP_FADEIN_DURATION = 50;
	PLMXL.HELP_FADEOUT_DURATION = 300;
	PLMXL.HELP_TEXT_RESPONSE_TIME = 500;
	PLMXL.HELP_TEXT_RESPONSE_TOUT;
	...
	PLMXL.PROJECT_WORKFLOW_BASEID = 18233;
	PLMXL.PROJECT_STATUS_BASEID = 18043;
...
function resizeWindow(e) {
	if(PLMXL.pinStack.length > 0) {
		...
		if($help.css("position") == "fixed") {
			...
			var opcode = jQuery('#opcode').val();
			var isCreatePopup = opcode == 'selectSubClassForPopup' || opcode == 'createNewObjectInPopup';
			setHelpHeight($help, null, $description, desiredHeight, isCreatePopup);
		}
	}
}
...
function bindHelpDt($thisDT, helpTextArray, attColorsArray, wfAttrId, status, isAffectedItem) {
	var attid = "";
	try {
		if(wfAttrId != null) {
			attid = wfAttrId + jQuery.trim(status);
	...
	if(attHelp != undefined) {
		...
		$hoverSpan.bind('mouseenter', function() {
			...
			PLMXL.HELP_TEXT_RESPONSE_TOUT = setTimeout(function () {
				...
				if(isAffectedItem) {
					if(PLMXL.statistics.affectedItem.attrs[key] == undefined) {
						PLMXL.statistics.affectedItem.attrs[key] = 1;
					} else {
						var count = PLMXL.statistics.affectedItem.attrs[key];
						count++;
						PLMXL.statistics.affectedItem.attrs[key] = count;
					}
				} else {
					if(PLMXL.statistics.main.attrs[key] == undefined) {
						PLMXL.statistics.main.attrs[key] = 1;
					} else {
						var count = PLMXL.statistics.main.attrs[key];
						count++;
						PLMXL.statistics.main.attrs[key] = count;
					}
				}

				mouseEnterDT($this, hText, attid, fontColor, backgroundColor);

			}, PLMXL.HELP_TEXT_RESPONSE_TIME);
		}).bind('mouseleave', function() {
			clearTimeout(PLMXL.HELP_TEXT_RESPONSE_TOUT);
			mouseLeaveDT($hoverSpan, attid);
		});

		$hoverSpan.bind("contextmenu", function(evt) {
			evt.preventDefault();
			pinHandler(null, null, attid, hText, fontColor, backgroundColor, true, status);
		});

		var opcode = jQuery('#opcode').val();
		var isCreatePopup = opcode == 'selectSubClassForPopup' || opcode == 'createNewObjectInPopup';
		if(!isCreatePopup && PLMXL.IS_COVER_PAGE) {
			$hoverSpan.bind('click', function() {
				maximize(null, null, attid, status);
			});
		}
	}
}
...
function mouseEnterDT($this, hText, attid, fontColor, backgroundColor) {
	if(hText != undefined) {
		...
		var dtSpanRight = $this[0].getBoundingClientRect().right;
		...
		if(PLMXL.helpMap[attid] != undefined) {
			...
			$description = $help.find('.description');
			var isSpanRightUsed = setHelpWidth($help, dtSpanLeft, $help.outerWidth(), isCreatePopup, dtSpanRight);
			setHelpHeight($help, $this, $description, $help.outerHeight(), isSpanRightUsed);
			setMaxButton($help, $description, isCreatePopup);
		} else {
			$this.css('background-color',PLMXL.hoverBackGroundColor);
			...
			var isSpanRightUsed = setHelpWidth($help, dtSpanLeft, PLMXL.HELP_WIDTH, isCreatePopup, dtSpanRight);

			setHelpHeight($help, $this, $description, PLMXL.HELP_HEIGHT, isSpanRightUsed);

			setMaxButton($help, $description, isCreatePopup); // Should be called after setting size
...
function addEmptyHelpDiv (attid) {
	var newDIV = jQuery("<div class='help-toolTip' id='help-toolTip"+attid+"' style='display:none;'>" +
			...
			"<div class='help-footer' id='help-footer'>Assist + &copy Xavor Corporation</div>" +
			"</div>" +
	"</div>");	
	return newDIV.appendTo("body");
}

function addEmptyMessageDiv () {
	var newDIV = jQuery("<div class='help-toolTip' id='help-toolTipNotificationMsg' style='display:none;'>" +
	"<div>"+
	"<div class='description' id='helpDiscription'>Some detailed Text</div>" +
	"</div>" +
	"</div>");
...
function setHelpWidth(helpDIV, dtSpanLeft, desiredWidth, isCreatePopup, dtSpanRight) {
	var helpLeft;
	var isSpanRightUsed = false;

	if(desiredWidth < PLMXL.HELP_WIDTH) {
		desiredWidth = PLMXL.HELP_WIDTH;
	}
	...
	if(helpDIV.css("position") == "fixed") {
		...
		if(isCreatePopup) {
			if(desiredWidth < 192) {
				desiredWidth = calculateHelpWidth(dtSpanRight, desiredWidth);
				helpLeft = dtSpanRight - desiredWidth;
				isSpanRightUsed = true;
			} else {
				var orgDesiredWidth = 0 + desiredWidth;
				desiredWidth = calculateHelpWidth(dtSpanLeft, desiredWidth);
				if(desiredWidth < 192) {
					desiredWidth = calculateHelpWidth(dtSpanRight, orgDesiredWidth);
					helpLeft = dtSpanRight - desiredWidth;
					isSpanRightUsed = true;
				} else {
	...
	helpDIV.css('width', desiredWidth - PLMXL.HELP_WIDTH_VARIANCE).css("left", helpLeft + "px");
	return isSpanRightUsed;
}
...
function setHelpHeight(helpDIV, dtDIV, descriptionDIV, desiredHeight, isCreatePopup) {
	var windowHeight = window.innerHeight || (PLMXL.CSS1 ? document.documentElement.clientHeight : document.body.clientHeight);

	var topPosition;

	if (desiredHeight < PLMXL.HELP_HEIGHT) {
		desiredHeight = PLMXL.HELP_HEIGHT;
	}
	...
	if (desiredHeight < PLMXL.HELP_MIN_HEIGHT) {
		desiredHeight = PLMXL.HELP_MIN_HEIGHT;
	}
	...
	if(helpDIV.css("position") == "fixed") {
		...
		if(isCreatePopup) {
			topPosition = dtDIV[0].getBoundingClientRect().bottom;
		} else {
	...
	helpDIV.css("height", desiredHeight - PLMXL.HELP_HEIGHT_VARIANCE).css("top", topPosition + "px");	
	descriptionDIV.css('height', helpDIV.outerHeight() - PLMXL.HELP_HEADER_AND_FOOTER_HEIGHT);	
}

function setMaxButton(helpDIV, desciptionDIV, isCreatePopup) {
	...
	} else if(hasHorizontalScrollbar(desciptionDIV) || hasVerticalScrollbar(desciptionDIV) || helpDivHeight > PLMXL.HELP_HEIGHT || helpDivWidth > PLMXL.HELP_WIDTH) {
		maxButton.show();
...
function showToolTop($help, attid) {
	...
	$help.stop().fadeIn(PLMXL.HELP_FADEIN_DURATION).css('opacity', '1.0');
...
function pinHandler($help,$pinButton,attid,hText,fontColor,backgroundColor,flag,text)
{
	...
	{	
		...
		//If creation pop-up screen then decrease top positioning
		var opcode = jQuery('#opcode').val();
		if(opcode == 'selectSubClassForPopup' || opcode == 'createNewObjectInPopup') {
			pinTop -= 37;
		}
...
function hideToolTip($help, attid) {
	jQuery($help).fadeOut(PLMXL.HELP_FADEOUT_DURATION, stopVideo(attid));
}
...
function hasHorizontalScrollbar($description) {
	//Check if IE and Zoom lvl is not 100%
	var ua = window.navigator.userAgent;
	var old_ie = ua.indexOf('MSIE ');
	var new_ie = ua.indexOf('Trident/');
	if ((old_ie > -1) || (new_ie > -1)) {
		var zoomlvl = screen.deviceXDPI / screen.logicalXDPI;
		if(zoomlvl != 1) {
			var diff = $description.get(0).scrollWidth - $description.width();
			if(diff < 0) diff *= -1;
			return diff > 1;
		}
	}
...
function hasVerticalScrollbar($description) {
	...
	if ((old_ie > -1) || (new_ie > -1)) {
		var zoomlvl = screen.deviceXDPI / screen.logicalXDPI;
		if(zoomlvl != 1) {
			var diff = $description.get(0).scrollHeight - $description.height();
			if(diff < 0) diff *= -1;
			return diff > 1;
		}
	}
...
function loadAssistPlus() {
	try {
		...
		if(classid != undefined && classid != "") {
			var ride = classMap.has(classid) ? !classMap.get(classid)[1]: null;
			if((ride == null && NotificationMsg) || (ride != null && ride && NotificationMsg)) {
				if(NotificationMsg) {
					jQuery("#assistDummyMsg").remove();
					jQuery("#assistMsg").remove();
					jQuery("#header_tab_wrapper").prepend("<div style='padding: 8px;' id='assistDummyMsg'>H</div>");
			...
			if(classid == classIdInUse || classMap.has(classid) ? classMap.get(classid)[0] : false) {
				jQuery("#assistClassDummyMsg").remove();
				jQuery("#assistAgileClassMsg").remove();
				jQuery("#header_tab_wrapper").prepend("<div style='padding: 8px;' id='assistClassDummyMsg'>H</div>");
			}
			jQuery('#rightScrollMenu').css('top', 'auto'); 
			getAssistText(classid);
			classIdInUse = classid;
		} else {
			if(NotificationMsg) {
				jQuery("#assistMsg").remove();
				jQuery("#assistDummyMsg").remove();
...
//	try{
//		var showPreferredInbox = eval('document.MainForm.opcode').value;
//		if(showPreferredInbox=='showPreferredInbox'){
//			getNotificationMsg();
//		}
//	}catch(err){}
}
...
function getAssistText(classid) {
	...
	var isProject = jQuery("form[name=MainForm] #col_18041").text() != "";
	var wfId = "";
	try {
		var wf;
		if(isProject) {
			wf = jQuery("form[name=MainForm] #col_"+PLMXL.PROJECT_WORKFLOW_BASEID);
		} else {
			wf = jQuery("form[name=MainForm] #col_"+PLMXL.WORKFLOW_BASEID);
		}
	...
	try {
		if(isProject) {
			statusId = jQuery("form[name=MainForm] #col_"+PLMXL.PROJECT_STATUS_BASEID).text();
		} else {
	...
	try	{
		if(isProject) {
			objID = jQuery("form[name=MainForm] #col_18041").text();
		} else {
	...
	if(typeof(JSON) == "undefined") {
		statString = PLMXL.toJSON(PLMXL.statistics);
	} else {
		statString = JSON.stringify(PLMXL.statistics);
	}
	...
	jQuery.post(arg.url, {
		...
		statistics: statString,
		rnd:Math.random()},
		function(response) {
			PLMXL.statistics = {};
			PLMXL.statistics.userid = response.userid;
			PLMXL.statistics.main = {};
			PLMXL.statistics.main.classid = classid;
			PLMXL.statistics.main.attrs = {};

			var enableOptOut = response.enableOptOut;
			var isOptedOut = response.isOptedOut;
			try{
				setNotificationMsg(response, false);
				if(response.isNotifOverrideEnabled=="Yes" && !(response.isClassNotifEnabled=="No"||response.classNotificationMsg=="" ||
				   response.classNotificationMsg==undefined || response.classNotificationMsg==null || response.classNotificationMsg=='null')) {
					//NotificationMsg = false;
					jQuery("#assistDummyMsg").remove();	
					jQuery("#assistClassDummyMsg").remove();
					jQuery("#header_tab_wrapper").prepend("<div style='padding: 8px;' id='assistClassDummyMsg'>H</div>");					
			...
			if(isOptedOut != undefined && isOptedOut == "true") {
				jQuery('form[name=MainForm] dt').each(function(index) {
					...
					var $hoverSpan = $thisDT.find('.hoverSpan');
					$hoverSpan.unbind('mouseenter').unbind('mouseleave').unbind('contextmenu').unbind('click');
					$hoverSpan.css('cursor','');
					$hoverSpan.attr('title', '');
				...
				$showAssistText.css("cursor", "pointer");
			...
			.attr("flag", true)
			.attr("class", "showAssistText");

			if(enableOptOut) {
				$showAssistText.attr("title", "Click to disable Assist+ Help.")
			...
			if(affectedItemAssist) {
				//Statistics for Reports
				PLMXL.statistics.affectedItem = {};
				PLMXL.statistics.affectedItem.classid = affectedItemAssist.classid;
				PLMXL.statistics.affectedItem.attrs = {};
				...
				jQuery("#rightpanecontentsubTabs dt").each(function(index) {
					...
					bindHelpDt($thisDT, rlHelpText, rlAttColorsData, null, null, true);
				});
			...
			if(mainObjectAssist) {
				...
				jQuery("form[name=MainForm] dt").each(function(index) {
					...
					bindHelpDt($thisDT, helpText, attColorsData, null, null, false);
				});

				jQuery("form[name=MainForm] #WorkflowDiagramTable td").each(function(index) {
					var $thisDT = jQuery(this);
					var statusName = $thisDT.text();
					if(statusName != undefined && statusName != "") {
						var $workflow = jQuery('#WorkflowDiagram_toggle');
						var workflowName = jQuery.trim($workflow.text().substring($workflow.text().lastIndexOf("(")+1,$workflow.text().lastIndexOf(")")));
						if (PLMXL.prevColor == undefined) {
							PLMXL.prevColor = $thisDT.css('background-color');	
						}
						bindHelpDt($thisDT, helpText, attColorsData, workflowName, statusName, false);
					}
				});
...
function cacheAffectedItems(colNums) {
	...
	jQuery('#CHANGETABLE_AFFECTEDITEMS .GMBodyMid .GMDataRow').each(function(index) {
		...
		var itemNumber = $affectedItemRows.eq(colNums[0]).text().trim().replace(String.fromCharCode(160), '');
		var jObj = {};
