...
//			homePath = System.getProperty(Constants.Config.USERHOME_PROP); //"user.home"
			if ((homePath == null) || homePath.isEmpty()) {
				throw new Exception("PLMFLEX_HOME: environment variable not configured prperly. Please configure and restart weblogic for changes to take effect.");
			}
		}
	...
	public static Properties loadPropertyFile() throws Exception
	{
		...
		try(FileInputStream file = new FileInputStream(homePath+Constants.General.PropertyFileName)) {
			prop.load(file);
		} catch (Exception e) {
			e.printStackTrace();
		}
