	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {			
			...
			if(!userIdFromRequest.equalsIgnoreCase("null") && !userIdFromRequest.isEmpty() ) {
				...
				log.debug("Requesting page title: "+ request.getParameter("title"));
				userid=CommonUtils.getUserIdOfLoggedInUser(request, userIdFromRequest, log);
				if(userid.isEmpty()) {
					return;
				}
			}
			...
			if (!classId.equals("undefined")) {
				...
				if (murl != null && !murl.equals("") && !murl.equals("NULL_VALUE")) {
					...
					if (!isOptedOut) {
						...
						rolesList = AgileHandler.getCurrentUserRoles(session, userid);
					...
					try {
						...
						NotificationMsgBar notifBar=new NotificationMsgBar();
						notifBar.getNotificationDetails(dbh, jobj, rolesList, allRoleKey, userid, noOfChars);
						
					}catch (Exception e) {
						log.error("Error while loading the assist msg bar: ", e);
					}
					
					try {
						ClassNotificationMsgBar classNotifBar = new ClassNotificationMsgBar();
						classNotifBar.getClassNotificationMsgFromDB(dbh, jobj, classId, rolesList, allRoleKey);
						
					}catch (Exception e) {
						log.error("Error while loading the class assist msg bar: ", e);
					}
				}
			}
...
//	private String getNotificationBarMsgFromDB(DBHandler dbh) {
//		Map<?, ?> helpMap = dbh.handleDBRequest("getNotificationMsg", null, false);
//		
//		return helpMap.get("isNotifEnabled").equals("True")? helpMap.get("notificationMsg")+"":"";
//	}
