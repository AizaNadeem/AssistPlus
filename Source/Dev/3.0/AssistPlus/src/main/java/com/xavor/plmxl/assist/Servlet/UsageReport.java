...
public class UsageReport extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		log.debug("Entering UsageReport: doPost...");
		String jsonResponse = "";
		
		try {
			DBHandler dbh = new DBHandler();
			Map<String, List<JSONObject>> report = (Map<String, List<JSONObject>>) dbh.handleDBRequest("generateUsageReport", null, false);
			jsonResponse = new Gson().toJson(report);
		} catch(Exception ex) {
			log.error("Error while generating usage report: ", ex);
		...
		log.debug("Exiting UsageReport: doPost...");
	}
