$.template("classRow","<tr id=${idVal} data-level=${level} class=${classVal}> "+ /*>*/
		...
		"<td id='hasTextFlag'>{{if hasTextFlag=='yes'}}<img id='textImage' src='img/dot.gif' alt='Has Assist Text' title='Has Assist Text'>{{/if}}</td></tr>");


$.template("attributeRow", "<tr id=${attrIdVal} data-isWorkflow=${isWorkflow} data-attr=${attrIdVal} data-attrcolors=${assistColorId};${assistColor} class='attrRow'" +
		"{{if hasTextFlag=='yes'}} " +
		...
		"<td id=HasAssistText >" +
		"{{if hasTextFlag=='yes'}}" +
		"<img id='textImage' src='img/dot.gif' alt='Has Assist Text' title='Has Assist Text, Click to view already added texts. Right click to add text.'>" +
		"{{/if}}" +
		"</td>"+
