...
(function(){
	...
	//console.log(jQuery(head).find("script"));
	
	var assistPropertiesScript=document.createElement("script");
	assistPropertiesScript.type="text/javascript";
	assistPropertiesScript.src=homeDirectory+"/js/AssistProperties.js";
	
	head.appendChild(assistPropertiesScript);
	
	//console.log ("Script added tp head");
...
//		$thisDT.attr('title', 'Left click to maximize/minimize. \nRight Click to pin/unpin.');
		...
		$thisDT.bind('mouseenter',function(){
			var $this=jQuery(this);
			mouseEnterDT($this,hText,attid,fontColor,backgroundColor);
		});

		$thisDT.bind('mouseleave',function(){ 
			mouseLeaveDT($thisDT,attid);
		});
		$thisDT.bind("contextmenu", function(evt) {evt.preventDefault();pinHandler(null,null,attid,hText,fontColor,backgroundColor,true);});
		$thisDT.bind('click',function(){maximize(null,null,attid);});
...
function mouseLeaveDT($thisDT,attid)
{
	...
	if(PLMXL.helpMap[attid]!=undefined)
		...
		jQuery(PLMXL.helpMap[attid]).fadeOut(PLMXL.FADE_DURATION);
		}
}
...
function showToolTop($help,attid)
{
	...
	$thisDT.css('background-color',PLMXL.hoverBackGroundColor);
}

function mouseleaveToolTip($help,attid)
{
	var $thisDT=(jQuery("form[name=MainForm] #col_"+attid)).prev();
	$thisDT.css('background-color',PLMXL.prevColor);
	if($help!=undefined)
		{
		if($help.css("position") == "fixed")
			{return;}
		jQuery($help).fadeOut(PLMXL.FADE_DURATION);
		}
...
function pinHandler($help,$pinButton,attid,hText,fontColor,backgroundColor,flag)
{
	...
	{	
		...
		jQuery($help).animate({"left":pinLeft,"top":pinTop}, 50,
        					 function(){
...
function unPin($help,attid,hText,fontColor,backgroundColor,flag)
{
	...
	{	
		var $thisDT=(jQuery("form[name=MainForm] #col_"+attid)).prev();
		mouseEnterDT(jQuery($thisDT),hText,attid,fontColor,backgroundColor);
	}
	else
	{
		jQuery($help).fadeOut(PLMXL.FADE_DURATION);
