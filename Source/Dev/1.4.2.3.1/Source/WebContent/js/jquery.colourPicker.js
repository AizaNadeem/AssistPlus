...
jQuery.fn.colourPicker = function (conf) {
	...
	return this.each(function () {
		...
		var input	= jQuery("<span name='" + select.attr('name') + "' style='display:block;background-color:" +config.bgcolor+";color:"+config.fontcolor+" ;min-width:70px' attr-data='" + select.val() + "'>"+(/*config.fontcolor=='#666' ? "Set Default":*/config.isBG==true?"Background Color":"Text Color")+"</span>").insertAfter(select);
		...
		jQuery('option', select).each(function () {
			...
			+ '" rel="' 
			+ hex 
			+ '"  style="margin:0;padding0;background: #' 
			+ colors[1] 
			+ '; color: #' 
			+  colors[0]
			+ ';">' 
			+ (/*colors[0] == '666' ? 'Set Default':*/config.isBG==true?"Background":"Text Color")+'</p>'
			+'</li>';
		});
		...
		input.click(
			...
			colourPicker.toggle(config.speed);
