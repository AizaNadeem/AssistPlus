...
public class Authenticate extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (uid == -1) {
			...
			if(DBHandler.executeUpdate==false)
			{
				json = new Gson().toJson(new ReturnStatus("success", "Welcome Admin", responseJson));
			}
			else
			{
				DBHandler.executeUpdate=false;
				json = new Gson().toJson(new ReturnStatus("success", "Welcome Admin. Database Upgraded to Version 1.5.1", responseJson));
			}
