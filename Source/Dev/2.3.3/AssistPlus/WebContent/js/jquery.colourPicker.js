...
jQuery.fn.colourPicker = function (conf) {
	...
	var config = jQuery.extend({
		...
		title:		'Pick a theme',		// Default dialogue title
	...
	if (!colourPicker.length) {
		colourPicker = jQuery('<span id="' + config.id + '">&nbsp;</span>').appendTo(document.body)/*.hide()*/;
	...
	return this.each(function () {
		...
		var input	= jQuery("<button name='" + select.attr('name') + "' style='display:block;background-color:" +config.bgcolor+";color:"+config.fontcolor+" ;width:100%' attr-data='" + select.val() + "'>"+(/*config.fontcolor=='#666' ? "Set Default":*/config.isBG==true?"Background Color":"Text Color")+"</button>").insertAfter(select);
	
		var loc		= '';
		jQuery('option', select).each(function () { //TODO
			var option	= jQuery(this);
			var hex		= option.val();
			var colors =option.val().split("^");

			var title	= option.text();

			loc += '<li rel='+hex+'><button class=font href="#" title="' 
			+ title 
			+ '" rel="' 
			+ hex ;
			if(config.isBG!=true)
			{
				loc +='"  style="height:18px;border: 0 !important;width:80px;margin:0;padding0;background: #';
			}
			else
			{
				loc +='"  style="height:18px;border: 0 !important;width:84px;margin:0;padding0;background: #';
			}
			loc += colors[1] 
			+ '; color: #' 
			+  colors[0]
			...
			if(config.isBG!=true)
			{
				if(config.dfColor!=true && '#'+colors[1].toLowerCase()==backgroundColor.toLowerCase() && '#'+colors[0].toLowerCase()==fontColor.toLowerCase())
				{
					loc +='  Selected  </button>';
				}
				else
				{
					loc +='Text Color</button>';
				}								
			}
			else
			{
				loc +='Background</button>';
			}
			//loc +=(/*colors[0] == '666' ? 'Set Default':*/config.isBG==true?"Background":"Text Color")+'</p>'
			loc+='</li>';
		});
		...
		//+ colors[0] == '#666' ? 'Default Color':'Text Color'+'</p>'
		...
		if (config.inputBG) {
			input.change(function () {
				input
				.css('background-color','#' + input.attr('attr-data'))
				.addClass('picker');
			});
		...
		input.click(
			...
			//input.find('button:first').focus();
			
			 
			var heading	= config.title ? "<h2>" + config.title + "<div class='pickerClose ui-icon ui-icon-closethick pointer'></div></h2>" : "";
			...
			if((top+302)>windowHeight)
			{				
				if(config.isBG==true)
				{
					top=(top+27)-224;
				}
				else if(config.isLabel==true)
				{
					top=(top+27)-200;
				}
				else
			...
			colourPicker.find("button:eq(0)").focus();
			...
			// When you scroll the table
			$("#attr-div").scroll(function() {
				colourPicker.hide(config.speed);
			 });
			$("#text-div").scroll(function() {
				colourPicker.hide(config.speed);
			 });
			$(".rolesContainer").scroll(function() {
				colourPicker.hide(config.speed);
			 });
			
			jQuery('.pickerClose', colourPicker).click(function () {
				colourPicker.hide(config.speed);
				return false;
			});
			// When you click a colour in the colour-picker
			jQuery('li', colourPicker).click(function () {
				var $this=$(this);
				var $Select=$(this).find('button');

				input.css('color',$Select.css('color'));
				input.css('background-color',$Select.css('background-color'));
				if(config.isBG!=true && config.isLabel!=true) //TODO
				{						
					if( (config.dfColor==true ||( AssistColorPicker.getHexColor($Select.css('background-color'))==backgroundColor.toLowerCase() && AssistColorPicker.getHexColor($Select.css('color'))==fontColor.toLowerCase()) && config.rolesScreen!=true ) )
					{
						input.html("Default Color");
					}
					else
					{
						input.html("Text Color");
					}
