(function ($) {
    $.fn.autolist = function (options) {
        ...
        var _uid = function () {
                if (typeof jQuery._$uid == 'undefined') jQuery._$uid = Math.random() && 0x10000;
                return ++jQuery._$uid;
            };
        ...
        var _insert = function (uniqid, label) {
                ...
                $('<span />').prependTo($('<li />').text(label).addClass('ui-autolist-selected ui-helper-reset ui-corner-all ui-state-default').addClass(uniqid).attr('uniqid', uniqid).appendTo($ul)).addClass('ui-autolist-remove ui-helper-reset ui-corner-all ui-icon ui-icon-circle-close').attr('uniqid', uniqid).bind('click', function () {
        ...
        var _load = function () {
                ...
                $.getJSON(settings.json, function (ret) {
                    $(ret).each(function () {
                        $('<option />').attr('value', this.value).prop('selected', ($this.check ? true : false)).text(this.label).appendTo($this);
                    });
                    _init();
                });
            };
        var _init = function () {
                ...
                $ul = $('<ul />').addClass('ui-helper-reset ui-autolist-values').prependTo($wrap);
                $this.after($wrap);
                $input = $('<input />').addClass('ui-helper-reset ui-widget ui-autolist-input').bind('keydown', function (e) {
