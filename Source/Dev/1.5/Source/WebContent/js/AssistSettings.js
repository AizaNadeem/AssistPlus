				...
				// Iterate over the JSON array.
						if (roleentry.Priority < 0) {
							...
							if(roleentry.fontcolor==' ' && roleentry.background==' ')
								AssistColorPicker.createPicker($list.find('.roleText'),fontColor.value.toString(),backgroundColor.value.toString(),false,true);
							else
								AssistColorPicker.createPicker($list.find('.roleText'),roleentry.fontcolor,roleentry.background,false,true);
	...
	function stopped(event,ui){
		...
		if(ui.item.parents('ul#assist-roles').length > 0){
			if(ui.item.find('.colorSelector').length==0){
				...
				AssistColorPicker.createPicker(ui.item.find('.roleText'),fontColor.value.toString(),backgroundColor.value.toString(),false,true);
			    if($.browser.msie && $.browser.version == "7.0")
				{
