...
PLMFlex.Assist.AssistText=(function(){
		...
		$(window).resize(function(){
			...
			$('#text-table tr').each(function(index)
			{
					...
					if(width<200)
					{
						...
						//atRow.find("#rowText").width('200px');
						width=200;
					}
					else
					{
	...
	function loadAttributes(classId, classLevel,$this) 
	{
		...
		width=width-69;
		height=height/4;
		$AttContainer.append('<div id="waitingDiv" ><img src="img/loading1.gif" class="waiting" /></div> ');		
		...
		PLMFlex.Assist.Request.get("loadAttributes?classId=" + classId + "&level="+ classLevel ,function(jsonResponse){
			...
			if (jsonResponse.hasOwnProperty("object"))
			{
				...
				{
					var row=$(this);
					//console.log(row.data('attrcolors'));
	...
	function editText($this,dataObj)
	{
		...
		$dialogHTML.dialog({
			...
			open: function(){
				enableRichText($(this));
				//$("#mce_text_ifr").css('line-height','233px');
			},
			close:function(){
				$( this ).dialog( "destroy" );
				var $thisrow=$AttContainer.find('tr.selected');
	...
	//	$('#mce_text').css('line-height','233px');
		$this.find('textarea').tinymce({
			...
			width:"677",
			// Theme options
			theme_advanced_buttons1 : "save,newdocument,|,bold,underline,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull,fontselect,fontsizeselect",
