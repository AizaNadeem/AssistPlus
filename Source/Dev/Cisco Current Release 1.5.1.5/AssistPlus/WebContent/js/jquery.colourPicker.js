...
jQuery.fn.colourPicker = function (conf) {
	...
	return this.each(function () {
		...
		var input	= jQuery("<span name='" + select.attr('name') + "' style='background-color:" +config.bgcolor+";color:"+config.fontcolor+" ;width:70px' attr-data='" + select.val() + "'>"+(config.fontcolor=='#666' ? "Set Default":"Text Color")+"</span>").insertAfter(select);
		var loc		= '';
		...
		jQuery('option', select).each(function () {
			...
			loc += '<li rel='+hex+'><p class=font href="#" title="' 
					...
					+  colors[0]
					+ ';">' 
					+ (colors[0] == '666' ? 'Set Default':'Text Color')+'</p>'
					+'</li>';
		});
