...
PLMFlex.Assist.AssistText=(function(){
	...
	function checkDuplicateEntry(textID)
	{
		var entryList=new Array;
		var duplicateEntryFound=false;		
		var workflowOrLifecycle=$($('#text-table tr').find(".wfcomboSelect").children()[0]).html();
		$('#text-table tr').each(function(index)
		{
			if(index%2==0)
				return true;
				...
				"Save":function(){
					...
					else
					{
						...
						if(tinyMiceText!="")
						{
								...
								tinyMiceText=tinyMiceText.replace(/<strong>/g,"<b>");
								tinyMiceText=tinyMiceText.replace(/<\/strong>/g,"</b>");
								tinyMiceText=tinyMiceText.replace(/<em>/g,"<i>");
								tinyMiceText=tinyMiceText.replace(/<\/em>/g,"</i>");
								tinyMiceText=tinyMiceText.replace(/<span>/g,"");
								tinyMiceText=tinyMiceText.replace(/<\/span>/g,"");
	...
	function setText(){
		...
		tinymce.get('mce_text').getBody().innerHTML = mce_text;
	}
	...
	//	$('#mce_text').css('line-height','233px');
		$this.find('textarea').tinymce({
			...
			width:"677",
			// Theme options
			theme_advanced_buttons1 : "save,newdocument,|,bold,underline,italic,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull,fontselect,fontsizeselect",
			theme_advanced_buttons2 : "cut,copy,paste,pastetext,pasteword,|,search,replace,|,|,undo,redo,|,link,unlink,anchor,image,help,code,|,insertdate,inserttime,preview,|,",
