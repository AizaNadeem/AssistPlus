		...
		PLMFlex.Assist.Request.get("loadConfig?",function(jsonResponse)
				{
						...
						jQuery('option', select).each(function () {
							...
							if(config.isBG!=true)
							{
								if(config.dfColor!=true && '#'+colors[1].toLowerCase()==backgroundColor.value.toLowerCase().toString() && '#'+colors[0].toLowerCase()==fontColor.value.toLowerCase().toString())
								{
									loc +='  Default  </button>';
								}
								else
		...
		input.click(
			...
			jQuery('li', colourPicker).click(function () {
				...
				PLMFlex.Assist.Request.get("loadConfig?",function(jsonResponse)
				{
					...
					fontColor=jsonResponse.fontColor;
					backgroundColor=jsonResponse.backgroundColor;
					if(config.isBG!=true && config.isLabel!=true)
					{						
						if( (config.dfColor==true ||( AssistColorPicker.getHexColor($Select.css('background-color'))==backgroundColor.value.toLowerCase().toString()  && AssistColorPicker.getHexColor($Select.css('color'))==fontColor.value.toLowerCase().toString()) && config.rolesScreen!=true ) )
						{
