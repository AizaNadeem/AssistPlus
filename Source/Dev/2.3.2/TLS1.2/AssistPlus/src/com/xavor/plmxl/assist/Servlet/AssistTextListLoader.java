...
public class AssistTextListLoader extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String errorMsg = (e != null) ? "" + e.getMessage() : "Null";
			json = new Gson().toJson(new ReturnStatus("error", "ServletError: " + errorMsg));
		}
		response.getWriter().write(json);
		log.debug(json.toString());
	...
	public String addNewRow(HttpServletRequest request) {
		...
		// The aim of manipulating data here is that so DB structure is not
		// changed
		// The workflowID is set to Lifecycles. When a request comes in
		// AssistPlus servlet, "Lifecycles" is also set as WorkflowID
		// For statuses, if the input is "All Lifecycles" then that is changed
		// to "All Statuses" as DB handles the All Statuses case
		// The workflowID becomes workflowStatusID to complete the manipulation
		// of the data.
		...
		try {
			...
			dbh.handleDBRequest("addNewAssistText", params, true);
	...
	private String deleteRow(HttpServletRequest request) {
		String textId = request.getParameter("textId");
		String json = "";
	...
	private String modeNew(HttpServletRequest request) {
		...
		try {
			dbh = new DBHandler();
			Map<String, String[]> map = (Map<String, String[]>) dbh.handleDBRequest("getRoleOptions", null, false);
			roles = map.get("strRoles");
