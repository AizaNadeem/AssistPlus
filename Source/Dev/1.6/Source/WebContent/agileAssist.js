...
function getAssistText(classid) {
	...
	jQuery.post(arg.url, {
		...
		userid:userid,
		objID:objID,
		isRequestForAffectedItems:"false",
		rnd:Math.random()},
		function(response) {
			...
			/**
			 * Load Assist Data for Affected Item
			 * 		Redlines if possible
			 * */
			...
			var rowNum = getSelectedRowNoUsingItemNumber(getItemNumberFromRedlinesTable());
			if(rowNum == -1) {
				return;
			}
			
			var rowData = getSelectedRowData(rowNum, colNums);
			var itemNumber = rowData[0];
			var oldLifecycle = rowData[1];
			var newLifecycle = rowData[2];

			var lifecyle = "";

			if(newLifecycle === ""){
				if(!(oldLifecycle === "") ){
					lifecyle = oldLifecycle;
				}
			} else{
			...
			jQuery.post(arg.url, {
				...
				workflowID:"",
				workflowStatusID:"",
				lifecycleID:lifecyle,
				userid:userid,
				objID:itemNumber,
				isRequestForAffectedItems:"true",
				rnd:Math.random()},
				function(rlResponse) {
					var rlHelpText = rlResponse.helpText;
					var rlAttColorsData = rlResponse.attColors;
...
function getItemNumberFromRedlinesTable() {
	...
	try {
		...
		itemNumber = h4.text().trim().replace(/ /g, '').replace('Redlinesfor','');
	} catch(ex) {}
