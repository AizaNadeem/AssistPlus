			...
			if (userid == null || userid.trim().isEmpty()) {
				...
				if (cookies != null) {
					...
					if (agile_url != null) {
						...
						final IAgileSession cookiesSession = AgileUtils.getAgileCookieSession(cookies, agile_url);
						int maxThreadPool=Integer.parseInt(ConfigHelper.loadPropertyFile().getProperty("max.threads", "10"));
						int timeout=Integer.parseInt(ConfigHelper.loadPropertyFile().getProperty("Timeout.in.seconds", "5"));						
						if (cookiesSession != null) {
							...
							try { 
								Callable<String> callables = new Callable<String>() {
									public String call() throws Exception {
										String userid=cookiesSession.getCurrentUser().getValue(UserConstants.ATT_GENERAL_INFO_USER_ID).toString();
										return userid;
									}
								};
								Future<String> result = executorService.submit(callables);
								userid=result.get(timeout, TimeUnit.SECONDS);
								log.error("user id obtained: "+userid);
							...
							cookiesSession.close(true); //2.3.0.4
						} else {
