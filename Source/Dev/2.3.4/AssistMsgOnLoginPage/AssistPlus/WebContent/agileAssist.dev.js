...
function setNotificationMsg(response) {
	...
	if(response.isNotifEnabled=="No"||response.notificationMsg=="" ||
			...
			response.notificationMsg=='null'){
		return;
	}
	jQuery("#header_tab_wrapper").prepend("<div id='assistMsg'>"
	+response.notificationMsg+
	"</div>");
	
	if((typeof LoginForm!='undefined') && LoginForm){
		jQuery("#dms").append("<div id='assistMsg'>"
				+response.notificationMsg+
				"</div>"); 
	}
...
function getNotificationMsg() {
	var arg = {};
	arg.url = "/AssistPlus/NotificationMsgBar?buster="+new Date().getTime();;
	jQuery.get(arg.url, {},
			function(response) {
			setNotificationMsg(response);	
			}, "json");
}
function initPLMXL() {
	PLMXL.TOUT;
